...
const HUD: React.FC<HUDProps> = ({ syncP, scrollSpeed, onSpeedChange, sps}) => {
  ...
  <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-10 pointer-events-auto">
    <div className="bg-black/90 border border-yellow-500/30 backdrop-blur-md rounded-md flex flex-row items-center gap-10 font-mono text-yellow-500 !pl-10 !pr-10 py-4 shadow-2xl">
      ...
      <div className="flex items-center gap-4">
        ...
        <span className={`text-4xl font-black italic tabular-nums inline-block w-[4.5ch] text-right ${syncP > 85 ? 'text-green-400' : 'text-yellow-500'}`}>
          {syncP}%
        </span>
      </div>
      ...
      {/* SPS */}
      <div className="flex items-center gap-4">
        <span className="text-[20px] opacity-50 uppercase tracking-[0.2em]">Strafes/sec</span>
        <span className="text-4xl font-black italic tabular-nums text-yellow-500 inline-block w-[3.5ch] text-right">
          {sps.toFixed(1)}
        </span>
      </div>
      ...
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xl font-bold w-[4ch] tabular-nums inline-block text-right">
